...
// where n_{ion} is the number of ionizing photons per baryon. The averaged recombination time is
// given by t_{rec} ~ 0.93 Gyr * (C_{HII}/3)^-1 * (T_0/2e4 K)^0.7 * ((1+z)/7)^-3. We assume the
...
static bool RecombPhotonCons =
    false;  // apply recombinations to the expected (not calibration) neutral fraction
...
static double PhotonConsEnd =
    0.3;  // HI fraction to start the smoothing (not using the exact calibration)
...
int InitialisePhotonCons() {
    ...
    */
    ...
    Try {  // this try wraps the whole function.
        ...
        double a_start = 0.03,
               a_end =
                   1. / (1. + astro_params_global
                                  ->PHOTONCONS_CALIBRATION_END);  // Scale factors of 0.03 and 0.17
                                                                  // correspond to redshifts of ~32
        ...
        // We are going to come up with the analytic curve for the photon non conservation
        // correction This can be somewhat numerically unstable and as such we increase the sampling
        // until it works If it fails to produce a monotonically increasing curve (for Q as a
        // function of z) after 10 attempts we crash out
            ...
            while (a < a_end) {
                ...
                if (matter_options_global->SOURCE_MODEL > 0) {
                    ...
                    if (astro_params_global->ION_Tvir_MIN < 9.99999e3) {  // neutral IGM
                        M_MIN_z0 = (float)TtoM(z0, astro_params_global->ION_Tvir_MIN, 1.22);
                        M_MIN_z1 = (float)TtoM(z1, astro_params_global->ION_Tvir_MIN, 1.22);
                    } else {  // ionized IGM
                        M_MIN_z0 = (float)TtoM(z0, astro_params_global->ION_Tvir_MIN, 0.6);
                ...
                // Curve is no longer monotonically increasing, we are going to have to exit and
                // start again
                if (Q1 < Q1_prev) {
                ...
                if (da < 7e-5)
                    da = 7e-5;  // set minimum bin size.
                else
                    da = pow(da, reduce_ratio);
                Q0 = Q1;
                a = a + da;
            }
...
void determine_deltaz_for_photoncons() {
    ...
    double temp;
    float z_cal, z_analytic, NF_sample, NF_sample_min, gradient_analytic, z_analytic_at_endpoint,
        const_offset, z_analytic_2, smoothing_width;
    ...
    // extrapolation of the photon non-conservation correction
    delta_NF = 0.025;
    // A width (in neutral fraction data points) in which point we average over to try and avoid
    // sharp features in the correction (removes some kinks) Effectively acts as filtering step
    smoothing_width = 35.;

    // The photon non-conservation correction has a threshold (in terms of neutral fraction;
    // PhotonConsEnd) for which we switch from using the exact correction between the calibrated
    // (21cmFAST all flag options off) to analytic expression to some extrapolation. This threshold
    // is required due to the behaviour of 21cmFAST at very low neutral fractions, which cause
    // extreme behaviour with recombinations on

    // A lot of the steps and choices are not completely rubust, just chosed to smooth/average the
    // data to have smoother resultant reionisation histories
    ...
    if (!PhotonConsSmoothing) {
        ...
        // Determine the number of extrapolation points (to better smooth the correction) between
        // the threshod (PhotonConsEnd) and a point close to zero neutral fraction (set by
        // PhotonConsAsymptoteTo) Choice is to get the delta neutral fraction between extrapolated
        // points to be similar to the cadence in the exact correction
        if (calibrated_NF_min > PhotonConsAsymptoteTo) {
            N_extrapolated = ((float)N_NFsamples - 1.) * (NF_sample_min - calibrated_NF_min) /
                             (PhotonConsStart - NF_sample_min);
        } else {
            N_extrapolated = ((float)N_NFsamples - 1.) * (NF_sample_min - PhotonConsAsymptoteTo) /
                             (PhotonConsStart - NF_sample_min);
        }
        N_extrapolated =
            (int)floor(N_extrapolated) - 1;  // Minus one as the zero point is added below
    } else {
    ...
    if (calibrated_NF_min >= PhotonConsEnd) {
        ...
        // If all the values of delta z are increasing, then some of the smoothing of the correction
        // done below cannot be performed
        ...
        NeutralFractions[0] = 0.999 * NF_sample_min;
        if (increasing_val) {
            // Values of delta z are always increasing with decreasing neutral fraction thus make
            // the last point slightly larger
            // NOTE (jdavies): since Neutralfractions[] is increasing, isnt this backwards???
            deltaz[0] = 1.001 * deltaz[1];
        } else {
            // Values of delta z are always decreasing with decreasing neutral fraction thus make
        ...
        // endpoint The choice for doing so is to ensure the corrected reionisation history is
        // mostly smooth, and doesn't artificially result in kinks due to switching between how the
        // delta z should be calculated
        ...
        // Multiplitcation by 1.1 is arbitrary but effectively smooths out most kinks observed in
        // the resultant corrected reionisation histories
        gradient_analytic = 1.1 * (delta_NF) / (z_analytic - z_analytic_2);
        const_offset =
            (NeutralFractions[1 + N_extrapolated] + delta_NF) - gradient_analytic * z_analytic;
        ...
        for (i = 0; i < N_extrapolated; i++) {
            if (calibrated_NF_min > PhotonConsAsymptoteTo) {
                NeutralFractions[i + 1] = calibrated_NF_min + (NF_sample_min - calibrated_NF_min) *
                                                                  (float)(i + 1) /
                                                                  ((float)N_extrapolated + 1.);
            } else {
                NeutralFractions[i + 1] =
                    PhotonConsAsymptoteTo + (NF_sample_min - PhotonConsAsymptoteTo) *
                                                (float)(i + 1) / ((float)N_extrapolated + 1.);
            }
    ...
    // For some models, the resultant delta z for extremely high neutral fractions ( > 0.95) seem to
    // oscillate or sometimes drop in value. This goes through and checks if this occurs, and tries
    // to smooth this out This doesn't occur very often, but can cause an artificial drop in the
    // reionisation history (neutral fraction value) connecting the values before/after the photon
    // non-conservation correction starts.
    for (i = 0; i < (N_NFsamples + N_extrapolated); i++) {
        ...
        // Check if we have a neutral fraction above 0.95, that the values are decreasing (val2 <
        // val1), that we haven't sampled too many points (counter) and that the NF_sample_min is
        // less than around 0.8. That is, if a reasonable fraction of the reionisation history is
        // sampled.
        while (NeutralFractions[i + 1] > 0.95 && val2 < val1 && NF_sample_min < 0.8 &&
               counter < 100) {
    ...
    // If we are not increasing for all values, we can smooth out some features in delta z when
    // connecting the extrapolated delta z values compared to those from the exact correction (i.e.
    ...
    if (!increasing_val) {
        for (i = 0; i < (N_NFsamples + N_extrapolated); i++) {
            ...
            counter = 0;
            // Try and find a point which can be used to smooth out any dip in delta z as a function
            // of neutral fraction. It can be flat, then drop, then increase. This smooths over this
            // drop (removes a kink in the resultant reionisation history). Choice of 75 is somewhat
            // arbitrary
            while (val2 < val1 &&
                   (counter < 75 || (1 + (i + 1) + counter) > (N_NFsamples + N_extrapolated))) {
                counter += 1;
    ...
    for (i = 0; i < (N_NFsamples + N_extrapolated + 1); i++) {
        ...
        if (i == 0 || i == (N_NFsamples + N_extrapolated)) {
            ...
            // We are symmetrically smoothing, making sure we have the same number of data points
            // either side of the point we are filtering over This determins the filter width when
            // close to the edge of the data ranges
            if ((i - (int)floor(smoothing_width / 2.)) < 0) {
                smoothing_int = 2 * (i) + (int)((int)smoothing_width % 2);
            } else if ((i - (int)floor(smoothing_width / 2.) + ((int)smoothing_width - 1)) >
                       (N_NFsamples + N_extrapolated)) {
                smoothing_int =
                    ((int)smoothing_width - 1) -
                    2 * ((i - (int)floor(smoothing_width / 2.) + ((int)smoothing_width - 1)) -
                         (N_NFsamples + N_extrapolated)) +
                    (int)((int)smoothing_width % 2);
            } else {
            ...
            for (j = 0; j < (int)smoothing_width; j++) {
                if (((i - (int)floor((float)smoothing_int / 2.) + j) >= 0) &&
                    ((i - (int)floor((float)smoothing_int / 2.) + j) <=
                     (N_NFsamples + N_extrapolated + 1)) &&
                    counter < smoothing_int) {
...
void adjust_redshifts_for_photoncons(double z_step_factor, float *redshift, float *stored_redshift,
    ...
    *absolute_delta_z = 0.0;
    ...
    if (*redshift < astro_params_global->PHOTONCONS_CALIBRATION_END) {
        LOG_ERROR(
            "You have passed a redshift (z = %f) that is lower than the enpoint of the photon "
            "non-conservation correction "
            "(astro_params_global->PHOTONCONS_CALIBRATION_END = %f). If this behaviour is desired "
            "then "
            "set astro_params_global->PHOTONCONS_CALIBRATION_END "
    ...
    if (required_NF > PhotonConsStart) {
        ...
        *absolute_delta_z = 0.;
                ...
                *absolute_delta_z =
                ...
                if (!isfinite((double)*absolute_delta_z) || isnan((double)*absolute_delta_z)) {
                    LOG_ERROR(
                        ...
                        *absolute_delta_z, PhotonConsAsymptoteTo);
                    Throw(PhotonConsError);
                }
        ...
        // Note that this is deliberately tailored to light-cone quantites, but will still work with
        // co-eval cubes Though might produce some very minor discrepancies when comparing outputs.
    ...
    *stored_redshift = *redshift;
    ...
    // This redshift snapshot now uses the modified redshift following the photon non-conservation
    // correction
...
void z_at_Q(double Q, double *splined_value) {
    ...
    if (Q < Qmin) {
        ...
        LOG_ERROR(
            "This error can occur if astro_params_global->PHOTONCONS_CALIBRATION_END is close to "
            "the final sampled redshift. One can consider a lower value for "
            "astro_params_global->PHOTONCONS_CALIBRATION_END to mitigate this");
        //        Throw(ParameterError);
        Throw(PhotonConsError);
    } else {
...
void initialise_NFHistory_spline(double *redshifts, double *NF_estimate, int NSpline) {
    ...
    // Determine the point in the data where its no longer zero (basically to avoid too many zeros
    // in the spline)
    ...
    for (i = 0; i < (counter + 1); i++) {
        ...
        // At the extreme high redshift end, there can be numerical issues with the solution of the
        // analytic expression
        if (i > 0) {
...
// NOTE: The way I've set this up is a little annoying in that this function needs to match its
