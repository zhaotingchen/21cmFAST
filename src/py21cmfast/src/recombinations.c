...
double splined_recombination_rate(double z_eff, double gamma12_bg) {
    ...
    if (lnGamma < RR_lnGamma_min) {
        ...
        if (!oob_warning_printed) {
            LOG_WARNING(
                "splined_recombination_rate: Gamma12 of %g is outside of interpolation array",
                gamma12_bg);
...
//  input parameters are the background ionization rate, overdensity, temperature (in 10^4k),
...
double Gamma_SS(double Gamma_bg, double del, double T_4, double z) {
    ...
    return Gamma_bg *
           (0.98 * pow((1.0 + pow(del / D_ss, 1.64)), -2.28) + 0.02 * pow(1.0 + del / D_ss, -0.84));
}
...
double MHR_rr(double lnD, void *params) {
    ...
    PDelta =
        p.A *
        exp(-0.5 * pow((pow(del, -2.0 / 3.0) - p.C_0) / ((2.0 * 7.61 / (3.0 * (1.0 + z)))), 2)) *
        pow(del, p.beta);
