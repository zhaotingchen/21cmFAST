...
double transfer_function_BBKS(double k) {
    double gamma, q;
    gamma =
        cosmo_params_global->OMm * cosmo_params_global->hlittle *
...
double transfer_function_Peebles(double k) {
    ...
    bb = 4.7 / pow(cosmo_params_global->hlittle * gamma,
                   2);  // no ^2 in dsigmasq, but that makes no sense
...
/*
  this function reads the z=0 matter (CDM+baryons)  and relative velocity transfer functions from
  CLASS (from a file) flag_int = 0 to initialize interpolator, flag_int = -1 to free memory,
...
void TFset_parameters() {
    ...
    double obhh =
        cosmo_params_global->OMb * cosmo_params_global->hlittle * cosmo_params_global->hlittle;
    ...
    cosmo_consts.sound_horizon =
        2.0 / 3.0 / k_equality * sqrt(6.0 / R_equality) *
    ...
    alpha_nu *=
        1 + (p_cb - p_c) / 2.0 * (1.0 + 1.0 / (4.0 * p_c + 3.0) / (4.0 * p_cb + 7.0)) / (1.0 + y_d);
...
double M_J_WDM() {
    ...
    z_eq = 3600 * (cosmo_params_global->OMm - cosmo_params_global->OMb) *
           cosmo_params_global->hlittle * cosmo_params_global->hlittle / 0.15;
    return fudge * 3.06e8 * (1.5 / g_x) *
           sqrt((cosmo_params_global->OMm - cosmo_params_global->OMb) *
                cosmo_params_global->hlittle * cosmo_params_global->hlittle / 0.15) *
           pow(M_WDM, -4) * pow(z_eq / 3000.0, 1.5);
}
...
double omega_mz(float z) {
    return cosmo_params_global->OMm * pow(1 + z, 3) /
           (cosmo_params_global->OMm * pow(1 + z, 3) + cosmo_params_global->OMl +
            cosmo_params_global->OMr * pow(1 + z, 4) + cosmo_params_global->OMk * pow(1 + z, 2));
}
...
double dicke(double z) {
    ...
    if (fabs(cosmo_params_global->OMm - 1.0) < tiny) {  // OMm = 1 (Einstein de-Sitter)
        return 1.0 / (1.0 + z);
    } else if ((cosmo_params_global->OMl > (-tiny)) &&
               (fabs(cosmo_params_global->OMl + cosmo_params_global->OMm +
                     cosmo_params_global->OMr - 1.0) < 0.01) &&
               (fabs(cosmo_params_global->wl + 1.0) < tiny)) {
        // this is a flat, cosmological CONSTANT universe, with only lambda, matter and radiation
        // it is taken from liddle et al.
        omegaM_z = cosmo_params_global->OMm * pow(1 + z, 3) /
                   (cosmo_params_global->OMl + cosmo_params_global->OMm * pow(1 + z, 3) +
                    cosmo_params_global->OMr * pow(1 + z, 4));
        dick_z = 2.5 * omegaM_z /
                 (1.0 / 70.0 + omegaM_z * (209 - omegaM_z) / 140.0 + pow(omegaM_z, 4.0 / 7.0));
        dick_0 = 2.5 * cosmo_params_global->OMm /
                 (1.0 / 70.0 + cosmo_params_global->OMm * (209 - cosmo_params_global->OMm) / 140.0 +
                  pow(cosmo_params_global->OMm, 4.0 / 7.0));
        return dick_z / (dick_0 * (1.0 + z));
    } else if ((cosmo_params_global->OMtot < (1 + tiny)) &&
               (fabs(cosmo_params_global->OMl) <
                tiny)) {  // open, zero lambda case (peebles, pg. 53)
        ...
        return dick_z / dick_0;
               ...
               (fabs(cosmo_params_global->OMtot - 1.0) < tiny) &&
               (fabs(cosmo_params_global->wl + 1) > tiny)) {
        LOG_WARNING("IN WANG.");
        Throw(ValueError);
    }
...
double ddickedt(double z) {
    ...
    if (fabs(cosmo_params_global->OMm - 1.0) < tiny) {  // OMm = 1 (Einstein de-Sitter)
        ...
        omegaM_z = cosmo_params_global->OMm * pow(1 + z, 3) /
                   (cosmo_params_global->OMl + cosmo_params_global->OMm * pow(1 + z, 3) +
                    cosmo_params_global->OMr * pow(1 + z, 4));
        domegaMdz = omegaM_z * 3 / (1 + z) -
                    cosmo_params_global->OMm * pow(1 + z, 3) *
                        pow(cosmo_params_global->OMl + cosmo_params_global->OMm * pow(1 + z, 3) +
                                cosmo_params_global->OMr * pow(1 + z, 4),
                            -2) *
                        (3 * cosmo_params_global->OMm * (1 + z) * (1 + z) +
                         4 * cosmo_params_global->OMr * pow(1 + z, 3));
        dick_0 = cosmo_params_global->OMm /
                 (1.0 / 70.0 + cosmo_params_global->OMm * (209 - cosmo_params_global->OMm) / 140.0 +
                  pow(cosmo_params_global->OMm, 4.0 / 7.0));

        ddickdz =
            (domegaMdz / (1 + z)) *
            (1.0 / 70.0 * pow(omegaM_z, -2) + 1.0 / 140.0 +
             3.0 / 7.0 * pow(omegaM_z, -10.0 / 3.0)) *
            pow(1.0 / 70.0 / omegaM_z + (209.0 - omegaM_z) / 140.0 + pow(omegaM_z, -3.0 / 7.0), -2);
        ddickdz -= pow(1 + z, -2) /
                   (1.0 / 70.0 / omegaM_z + (209.0 - omegaM_z) / 140.0 + pow(omegaM_z, -3.0 / 7.0));
...
double hubble(float z) {
    return Ho * sqrt(cosmo_params_global->OMm * pow(1 + z, 3) +
                     cosmo_params_global->OMr * pow(1 + z, 4) + cosmo_params_global->OMl);
}
